...
- **A stream error with no child tag no longer reports a blank reason.**
  `stream_error_details/1` fell back with `get_first_child_tag(node) || "unknown"`,
  but that function returns `""` (never `nil`) for a childless node, and `""` is
  truthy in Elixir — so the `"unknown"` fallback could not fire for any input,
  and the reason reached the log and `last_error` empty. Found by making
  Dialyzer blocking; eight unreachable clauses went with it.
